...
\section{El por qué de el método de Taylor}
\label{2:sec:1}
  La función $p(x)=a_0+a_1x+a_2x^2+ ... +a_nx^n$, en la que los coeficientes $a_k$ son constantes, se llama polinomio de grado n.En particular $y=ax+b$ es un polinomio de grado 1, de los más sencillos, por lo que calcular su valor en fácil. Sin embargo, calcular el valor par
a otras funciones como log(x), sen(x), $e^X$, ... es mucho más complicado. Por tanto, se utilizan metodos desarrollados por el análisis matemático, como el método de Taylor.
...
Sin embargo, esto solo se da para polinomios que tengan su derivada hasta n, mientras que para los polinomios que
 tienen derivada (n+1)-ésima difieren de f(x) en una pequeña cantidad, que denominamos como el error.\par
